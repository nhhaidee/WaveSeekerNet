...
def main():
    ...
    train_path      = '/gpfs/fs7/grdi/genarcc/wp1/genomics_unit/IAV/Retrain/data/Full_Train_Data/04_HA_Protein/'
    test_path       = '/gpfs/fs7/grdi/genarcc/wp1/genomics_unit/IAV/Retrain/data/04_HA_Protein_Host/'
    model_save_path = '/gpfs/fs7/grdi/genarcc/wp1/genomics_unit/IAV/Revision_1/model_weight/04_HA_Protein_Host/'
    ablation_result = '/fs/vnas_Hcfia/orph/hon000/revision_1/results/04_HA_Protein_Host/'

    # train data pre 2020 (good quality)
    X_train = np.load(train_path + 'X_train_onehot.npy')
    y_train = np.load(train_path + 'y_train_host.npy')
    

    # test data post 2020 (good quality)
    X_test_high_quality = np.load(test_path + 'X_test_high_onehot.npy')
    y_test_high_quality = np.load(test_path + 'y_test_high.npy')
    
    
    # test data post 2020 (poor quality)
    X_test_low_quality_post2020 = np.load(test_path + 'X_test_low_onehot.npy')
    y_test_low_quality_post2020 = np.load(test_path + 'y_test_low.npy')
    ...
    for kfold_index, (train, test) in enumerate(splitter.split(X_train, y_train)):
        ...
        X_CV_train, y_CV_train = resampling(X_train[train], y_train[train], n_downsamples=16000, n_upsamples=600)
