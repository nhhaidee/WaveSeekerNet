...
class MakePatches(nn.Module):
    def __init__(self, patch_width, patch_height, stride, embedding_dim):
        super().__init__()
        ...
        self.embedding = nn.Linear(self.patch_width*self.patch_height, self.embedding_dim)
...
# class MakePatchesMultiChannels(nn.Module):
#     def __init__(self, patch_width, patch_height, stride, embedding_dim, n_channels=1):
#         super().__init__()
#         self.patch_width = patch_width
#         self.patch_height = patch_height
#         self.unfold = nn.Unfold((patch_width, patch_height), stride=stride)
...
class TransfomerBlock(nn.Module):
    def __init__(self, patch_size, num_classes, embedding_dim=128, patch_mode="patch", n_channels=1, use_fnet=False):
        ...
        #self.positional_encoding = PositionalEncoding(self.embedding_dim)
        #self.make_patches_multi_channels = MakePatchesMultiChannels(self.patch_width, self.patch_height, self.stride, self.embedding_dim, self.n_channels)
        self.make_patches = nn.ModuleList(
            [
                MakePatches(self.patch_width, self.patch_height, self.stride, self.embedding_dim)
...
#        x = self.positional_encoding(x)
        ...
        for i in range(self.n_channels):
            x_tmp = inputs[:, i:i+1, :, :]
            x_patch_c.append(self.make_patches[i](x_tmp))
        x = torch.concat(x_patch_c, dim=-2)
