...
from transformer_esm import TransformerClassifier
...
def main():
    ...
    path            = '/gpfs/fs7/grdi/genarcc/wp1/genomics_unit/IAV/Retrain/data_esm/2_Channels_Protein_Host/'
    ablation_result = '/fs/vnas_Hcfia/orph/hon000/retrain/transformer_esm/result/2_Channels_Protein_Host/'
    model_weights   = '/gpfs/fs7/grdi/genarcc/wp1/genomics_unit/IAV/Retrain/model_weight/transformer_esm/2_Channels_Protein_Host/'

    # train data pre 2020 (good quality)
    X_train = np.load(path + 'X_train_tokens.npy')
    ...
    for kfold_index, (train, test) in enumerate(splitter.split(X_train, y_train)):
        ...
        clf_esm.fit(X_train[train], y_train[train], X_train[test], y_train[test])
