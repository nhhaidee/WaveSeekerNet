...
def main():
    ...
    model_save_path = '/gpfs/fs7/grdi/genarcc/wp1/genomics_unit/IAV/Retrain/model_weight/2_Channels_Protein_Host/'
    ablation_result = '/fs/vnas_Hcfia/orph/hon000/retrain/results_no_gmlp/2_Channels_Protein_Host/'
    ...
    print("Test Low-quality Data Shape (Pre 2020) :", X_test_low_quality_pre2020.shape, y_test_low_quality_pre2020.shape)
    print("Test Low-quality Data Shape (Pre+Post 2020) :", X_test_low_quality.shape, y_test_low_quality.shape)
    
    

    n_out = len(np.unique(y_train))
    n_channels = 2
    seq_len = 2 * X_train.shape[3]
    res_len = 21
    ...
    param_results_low_quality_pre_2020 = []
    ...
    for kfold_index, (train, test) in enumerate(splitter.split(X_train, y_train)):
        ...
        np.save(model_save_path+model_name+"_low_quality_post2020_"+str(kfold_index)+".npy",ctest_test_low_quality_post2020)
        ...
        ctest_test_low_quality_pre2020 = clf.predict(X_test_low_quality_pre2020)
        np.save(model_save_path+model_name+"_low_quality_pre2020_"+str(kfold_index)+".npy",ctest_test_low_quality_pre2020)
        
        ba_test_3, f1_test_3, p_score_test_3, r_score_test_3, mcc_test_3 = get_score(y_test_low_quality_pre2020, ctest_test_low_quality_pre2020)
        param_results_low_quality_pre_2020.append((model_name, ba_test_3, f1_test_3, p_score_test_3, r_score_test_3, mcc_test_3))
        
        
        print("Low Quality (Pre+Post 2020)")
        ctest_test_low_quality = clf.predict(X_test_low_quality)
        np.save(model_save_path+model_name+"_low_quality_"+str(kfold_index)+".npy",ctest_test_low_quality)
        
        ba_test_4, f1_test_4, p_score_test_4, r_score_test_4, mcc_test_4 = get_score(y_test_low_quality, ctest_test_low_quality)
        param_results_low_quality.append((model_name, ba_test_4, f1_test_4, p_score_test_4, r_score_test_4, mcc_test_4))
        ...
        df_param_results_low_quality_post_2020.to_csv(ablation_result + "Ablation_Low_Quality_Post2020_Fold_" + str(kfold_index) + ".csv")
        ...
        df_param_results_low_quality_pre_2020 =  pd.DataFrame(param_results_low_quality_pre_2020, columns=cv_cols)
        df_param_results_low_quality_pre_2020.to_csv(ablation_result + "Ablation_Low_Quality_Pre2020_Fold_" + str(kfold_index) + ".csv")
        print ('\nLow Quality Pre 2020')
        print ('F1-Score (Macro)\n')
        print (df_param_results_low_quality_pre_2020.groupby(['Model'])['F1-Score (Macro)'].agg(['mean', 'std', 'sem', 'count']).to_string())
        print ('Balanced Accuracy\n')
        print (df_param_results_low_quality_pre_2020.groupby(['Model'])['Balanced Accuracy'].agg(['mean', 'std', 'sem', 'count']).to_string())
        print ('MCC\n')
        print (df_param_results_low_quality_pre_2020.groupby(['Model'])['MCC'].agg(['mean', 'std', 'sem', 'count']).to_string())
        
        df_param_results_low_quality =  pd.DataFrame(param_results_low_quality, columns=cv_cols)
        df_param_results_low_quality.to_csv(ablation_result + "Ablation_Low_Quality_Fold_" + str(kfold_index) + ".csv")
        print ('\nLow Quality Pre+Post 2020')
        print ('F1-Score (Macro)\n')
        print (df_param_results_low_quality.groupby(['Model'])['F1-Score (Macro)'].agg(['mean', 'std', 'sem', 'count']).to_string())
        print ('Balanced Accuracy\n')
        print (df_param_results_low_quality.groupby(['Model'])['Balanced Accuracy'].agg(['mean', 'std', 'sem', 'count']).to_string())
        print ('MCC\n')
        print (df_param_results_low_quality.groupby(['Model'])['MCC'].agg(['mean', 'std', 'sem', 'count']).to_string())
